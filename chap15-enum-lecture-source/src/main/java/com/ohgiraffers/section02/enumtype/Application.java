package com.ohgiraffers.section02.enumtype;

public class Application {

	public static void main(String[] args) {

		/* 수업목표. 열거형(Enum)을 이용하여 상수 열거 패턴의 단점을 해결하는 것을 이해할 수 있다. */
		/* 목차. 1. 열거 타입으로 선언된 인스턴스는 싱글턴으로 관리되며 인스턴스가 한 개 임을 보장한다. */
		Subjects subject1 = Subjects.JAVA;
		Subjects subject2 = Subjects.HTML;
		Subjects subject3 = Subjects.JAVA;
		
		if(subject1 == subject2) {
			System.out.println("두 과목은 같은 과목입니다.");
		} else {
			System.out.println("두 과목은 다른 과목입니다.");
		}

		if(subject1 == subject3) {
			System.out.println("두 JAVA는 동일하며 이는 싱글톤이다.");
		} else {
			System.out.println("미안하다 이거 보여주려고 어그로 끌었다.");
		}
		
		/* 설명. 단일 인스턴스입을 보장하기에 == 비교가 가능하다. */
		System.out.println(subject1 == Subjects.JAVA);
		
		/* 목차. 2. 이름 충돌 방지를 위해 접두사를 쓰지 않아도 Enum 타입별로 네임스페이스를 가진다. */
		/* 필기.
		 *  동일한 이름의 상수가 필요한 경우 네임스페이스를 다르게 한다.
		 *  public enum Backend { JAVA, ORACLE, JDBC, JAVASCRIPT }
		 *  public enum Frontend { HTML, CSS, JAVASCRIPT }
		 * */
		
		/* 목차. 3. toString()을 이용하여 문자열로 변경하기 쉽다. */
		/* 설명. 앞의 예제처럼 switch()문을 사용하지 않아도 됨. */
		System.out.println(Subjects.JAVA.toString());	// =====JAVA=====
		System.out.println(Subjects.JAVA.name());		// JAVA

		/* Note. name() vs toString()
		 *  name()
		 *   Enum의 이름을 그대로 문자열로 반환함.
		 *   상수의 정확한 이름을 얻고자 할 때 사용되며, 오버라이드(재정의)할 수 없음.
		 *  toString()
		 *   겉에서 보기에는 기본적으로 name()과 같은 값을 반환함.
		 *   사용자가 필요에 따라 오버라이드(재정의)하여 반환값을 조작할 수 있음.
		 * */
		
		/* 목차. 4. values()를 이용하여 상수값 배열을 반환하고 이를 이용해 연속처리할 수 있다. */
		Subjects[] subjects = Subjects.values();
		for(Subjects s : subjects) {
			System.out.println(s);
		}

		for (Subjects s : subjects) {
			System.out.println(s.toString());
			System.out.println("index: " + s.ordinal());
			System.out.println("name: " + s.name());
			System.out.println();
		}

		/* 목차. 5. 타입 안정성을 보장한다. */
		printSubject(Subjects.HTML);
//		printSubject(2);		// 컴파일 에러: Enumtype은 인스턴스이기 때문에 정수를 사용하는 경우 타입불일치
	}
	
	public static void printSubject(Subjects subject) {
		System.out.println(subject.toString());
	}

}
